...
class MovieListsViewController: UIViewController {
    ...
    struct Item: Hashable {
        ...
        init(movie: Movie? = nil, lists: [MovieList]? = nil, emptyStateMessage: String? = nil) {
            self.movie = movie
            self.lists = lists
    ...
    private func getLists() {
        ...
        if lists.isEmpty {
            items = [Item(emptyStateMessage: "There are no lists")]
        } else {
    ...
    private func refreshData() {
        ...
        if movies.isEmpty {
            let item = Item(emptyStateMessage: "The list is empty")
...
extension MovieListsViewController {
    ...
    @objc func deleteButtonTapped() {
        for indexPath in moviesToDelete {
            let item = self.currentSnapshot.itemIdentifiers(inSection: .movies)[indexPath.row]
        ...
        if let movies = coreDataManager.fetchMovies(in: currentlySelectedList ?? "") {
            if movies.isEmpty {
                self.isMoviesInCurrentListPresent = false
                let item = Item(emptyStateMessage: "The list is empty")
                self.currentSnapshot.appendItems([item], toSection: .movies)
            } else {
            ...
            cell.listTappedToDeleteSubject
                .sink { index in
                    ...
                    if movieLists.isEmpty {
                        ...
                        self.currentSnapshot.appendItems([Item(emptyStateMessage: "There are no lists")], toSection: .lists)
                    } else {
        ...
        dataSource = UICollectionViewDiffableDataSource(collectionView: collectionView) {
            ...
            case .lists:
                if self.isListsCountEqualToZero {
                    return collectionView.dequeueConfiguredReusableCell(using: defaultCellRegistration, for: indexPath, item: item.emptyStateMessage)
                } else {
